...
def main():
    ...
    if not os.path.exists('./Dataset/{}'.format(n_ros)):
        os.makedirs('./Dataset/{}'.format(n_ros))

    for seed in args.seed:
        U.set_seed(seed + 1000)
        env = CartPoleEnv(max_ep_len=ep_len, seed=seed)
        dataset = generate_dataset(env, q_net, n_ros=n_ros)
        with open('./Dataset/{}/CartPole-ep{}-tau{}-n{}-seed{}.pickle'.format(n_ros, ep_len, tau, n_ros, seed), 'wb') as f:
