...
import utils as U
from Model.CI_Ope_Class import CI_SGDA
...
def get_parser():
    parser = argparse.ArgumentParser(description='CI-SGDA')
    parser.add_argument('--iter', type = int, default = 400, help='training iteration')
    ...
    parser.add_argument('--qs', type = str, nargs='+', default = ['32', '32'], help='neural network size for q')
    parser.add_argument('--ws', type = str, nargs='+', default = ['32'], help='neural network size for w')

    parser.add_argument('--q-lr', type = float, default = 5e-3, help='learning rate for q')
    parser.add_argument('--w-lr', type = float, default = 5e-3, help='learning rate for w')
...
def main(args):
    ...
    sgda = CI_SGDA(obs_dim, act_dim, q_net=q_net,
            q_hidden_layers=args.qs, q_lr=args.q_lr, w_hidden_layers=args.ws, w_lr=args.w_lr,
            scale=args.scale, ep_len=args.ep_len, gamma=args.gamma, norm=norm)
    sess.graph.finalize()

    value_true = U.eval_policy_cartpole(env, q_net, ep_num=10, gamma=args.gamma, prt=True)

    log_name = 'log_seed{}_qs{}_ws{}.pickle'.format(args.seed, 'x'.join(args.qs), 'x'.join(args.ws))
    
    if args.bootstrap:
        mid_dir = 'bootstrap_log'
    else:
        mid_dir = 'log'
    logger = ope_log_class(path='./{}/{}/CI_OPE/{}/Dataset{}/{}'.format(mid_dir, args.n_ros, args.scale, args.dataset_seed, args.tau), name=log_name, tau=args.tau, env_name=env_name, value_true=value_true)
    ...
    for iter in range(args.iter):  
        for i in range(1, args.w_iter + 1):
            data_sup = sample_data(dataset, args.bs)
            data_inf = sample_data(dataset, args.bs)
            sup_w_loss, inf_w_loss = sgda.train_w(data_sup, data_inf)

            if i % w_prt_interval == 0:
                print('-------------------------------------')
                print('Iter ', base + i)
                print('Sup Loss ', sup_w_loss, '; Inf loss ', inf_w_loss)
                lower_q, upper_q, lower_bound, upper_bound = sgda.evaluation(dataset)
                print('True value: {}. \nEst Lower Bound: {}\nEst Upper Bound:{}'.format(
                        value_true, lower_bound, upper_bound))
                print('-------------------------------------\n\n')
        ...
        for i in range(1, args.q_iter + 1):
            data_sup = sample_data(dataset, args.bs)
            data_inf = sample_data(dataset, args.bs)
            sup_q_loss, inf_q_loss, sup_debug, inf_debug = sgda.train_q(data_sup, data_inf)
